...
### Predicting C388 activity retention

We evaluated whether each construct retained C388 editing relative to its matched control. “Unchanged” denotes a change at or above the selected boundary, including increases; it does not establish statistical equivalence to the control.
...
This model is a computational reanalysis of the recorded measurements, not a newly recovered pre-assay prediction. C295 and C871 below retain the original fixed-panel models; their separate later reruns are not substituted here.

### Fixed-panel reconstruction and prediction agreement

The original project workflow predicted candidates before wet-lab testing. The table combines the retained C295/C871 reconstruction with the new C388 reanalysis. Agreement remains **11/15 endpoint calls**: **5/5 at C295, 3/5 at C388 and 3/5 at C871**. These correlated calls span different endpoint labels and are not a pooled generalization estimate.
...
| C388 construct | Measured ΔE (pp) | Observed class | Prediction |
|---|---:|---|---|
...
### Nested leave-one-construct-out validation of C388
...
The procedure classified **48/76 constructs correctly (63.2%)**, with **60.4% balanced accuracy** and **0.6042 macro F1**. Its matched training-majority reference achieved **50/76 (65.8%)** and **61.8% balanced accuracy**. The procedure therefore did not outperform this baseline in nested leave-one-out validation.
...
Current C388 evidence: [fixed-panel results and nested leave-one-out records](../research/c388_binary/), including protocols, per-construct predictions, selected CP coordinates and the fitted model.
