...
ax.axhline(.5,color=DARK,lw=.8,ls='--');ax.set_ylim(0,1);ax.set_xticks(range(4),['Design '+x for x in d.design],rotation=20,rotation_mode='anchor',ha='right');ax.set_ylabel('Work score')
...
counts=matrix_counts(d.success,d.prediction,[0,1]);confusion_panel(axes[1],counts,['Non-work','Work'],'',3)
for ax,label in zip(axes,['a','b']):panel_label(ax,label)
from matplotlib.patches import Patch
axes[0].legend(handles=[Patch(color=ORANGE,label='Measured work'),Patch(color=BLUE,label='Measured non-work')],loc='lower left',bbox_to_anchor=(-.05,1.02),frameon=False,fontsize=6.5)
...
ax.set_xticks([0,1],['Non-work (n = 17)','Work (n = 3)']);ax.set_xlim(-.55,1.55);ax.set_ylabel('S12 · high-confidence nonlocal contacts / residue');ax.grid(axis='y',alpha=.18)
...
for i,(site,labels,display) in enumerate([('C295',['decrease','not_decrease'],['Decrease','No decrease']),('C388',['decrease','within_selected_interval','increase'],['Decrease','Within\ninterval','Increase']),('C871',['decrease','not_decrease'],['Decrease','No decrease'])]):
 q=p[p.endpoint==site];c=matrix_counts(q.true_class,q.predicted_class,labels);confusion_panel(axes[i],c,display,site,5);panel_label(axes[i],chr(97+i))
