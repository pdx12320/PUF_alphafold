...
| **3. 非 TRM 设计** | 在有利的 TRM 背景上，哪些周围残基值得进一步测试？ | 逆折叠模型推荐的突变及其组合 | 为下一轮实验提供 G0–G10 候选序列 |
...
## 模型二：连接 TRM 改变与报告系统编辑表现

### 背景：分别评价靶位点保留与旁观者编辑降低
...
我们将留出预测与 P9-GNS、P9-NPS、P9-NTQ、P8-GVE 和 P4-R5-SNE+P7-R5-SNE 的实验测量进行比较。五个构建均保留了较高的 C388 编辑水平，测得的 C295 和 C871 编辑率也均低于各自的匹配对照。

| 构建 | C295 编辑率 | C388 编辑率 | C871 编辑率 | ΔC295（pp） | ΔC388（pp） | ΔC871（pp） |
|---|---:|---:|---:|---:|---:|---:|
...
P8-GVE 在这组构建中实现了最大的 C871 降幅，同时保留 65.86% 的 C388 编辑率。P9-NTQ 的 C388 变化较小，两个旁观者位点的编辑均下降。双 SNE 构建将 C388 编辑率提高了 7.77 pp，同时使 C295 和 C871 分别下降 13.15 和 34.90 pp。

这些测量为后续工程优化确定了可用背景。分类器识别出了其中大部分已记录的终点类别，而各候选的具体编辑优势由实验测量提供依据。

![实测编辑百分比及相对匹配对照的变化](figures/fig4_model2_measured_and_control_delta.png)

*图 4. 五个 TRM 构建的实验编辑表现。绝对编辑百分比和相对匹配对照的变化分开展示。每个变化量均使用该构建对应的实验对照，不同实验组的对照有所不同。图中使用已归档的构建级汇总值，未推算误差线，也未作显著性判断。*

### 为湿实验提供的依据

P8-GVE 和 P9 变体可作为研究“保留靶位点活性并降低 C871 编辑”的背景。P4/P7 双 SNE 构建则提供了提高 C388 编辑的另一条路线。这些位置优先级具有实验依据；当前分类器尚未提供经验证的单残基重要性分数。

来源记录：[五构建的预测、阈值与测量](../../results_20260922/five_construct_holdout/predictions.csv)、[完整构建集合的预测](../../results_20260922/v4/all81_LOCO.csv)和[匹配基线评价](../../results_20260922/v4/main_metrics.csv)。另有一项[十构建同时留出分析](../../results_20260922/ten_construct_holdout/ranking.csv)，采用不同评价方案，作为独立分析保留。

## 模型三：设计 TRM 识别编码之外的突变
...
| **5. 结合实验背景** | 将选定的非 TRM 替换加入 P8-GVE、P9-NTQ 或 P7-SYVIRR 背景 | G0–G10 共十一个候选序列 |
...
| 额外界面设计 | A438G、H392D | 在所述阈值下获得一个模型支持，纳入 G4 和 G8 |
| 额外 N 端设计 | S30A、L41R；N12S 为探索性设计 | 纳入几何组合；N12S 的推荐标签存在下述差异 |

![17 个双模型共识非 TRM 替换的采样支持](figures/fig5_model3_nontrm_consensus.png)

*图 5. 17 个共识替换的采样频率。每个数值表示某个模型生成的 10,000 条序列中，在相应位置携带所示残基的比例。该频率描述结构条件下的序列偏好，不能直接用于估计 RNA 编辑改善的概率。*
...
拟定的核心集合包含 T350V、D374E、M458L、L274I、T278I 和 V366I；界面集合包含 A438G 和 H392D；几何集合包含 N12S、S30A 和 L41R。这些集合名称表达设计假设，其具体分子效应仍需检验。

| 构建 | TRM 背景 | 额外非 TRM 替换 |
|---|---|---|
| G0 | P8-GVE | 无，作为匹配背景对照 |
| G1 | P8-GVE | R45T |
| G2 | P8-GVE | 核心集合 |
| G3 | P8-GVE | R45T + 核心集合 |
| G4 | P8-GVE | 界面集合 |
| G5 | P8-GVE | 几何集合 |
| G6 | P8-GVE | R45T + 核心 + 界面 + 几何集合 |
| G7 | P9-NTQ | R45T + 核心集合 |
| G8 | P9-NTQ | R45T + 界面集合 |
| G9 | P8-GVE + P9-NTQ | R45T + 核心集合 |
| G10 | P7-SYVIRR | 核心集合 |

下一轮实验应将每个组合与其对应的 TRM 背景比较。单突变比较有助于判断哪些替换贡献了实测变化。本次发布中的 G0–G10 为计算设计，其非 TRM 效应仍待测量。

来源记录：[采样与筛选方法](../../aice_mpnn_20260922/README.md)、[493 位置扫描](../../aice_mpnn_20260922/results/aice_single_ranked.csv)、[推荐突变导出表](../../aice_mpnn_20260922/results/recommended_mutations.csv)、[G0–G10 突变列表](../../aice_mpnn_20260922/results/gen3_constructs.csv)和[候选序列](../../aice_mpnn_20260922/results/gen3_constructs.fasta)。
