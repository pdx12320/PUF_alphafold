...
*图 1. 两种评价方案下的骨架分类结果。（a）扩展集合的逐构建留一评价：22/24 正确。（b）四个构建同时留出的评价：3/4 正确，包括一个真阳性、两个真阴性和一个假阳性。行表示实验记录类别，列表示九特征随机森林在 0.5 分数阈值下的预测类别。两个面板使用相同的计数色阶。历史决策树桩属于另一项分析。*

![scaffold_s12_distribution](figures/scaffold_s12_distribution.png)

*图 2. 扩展骨架集合中 S12 的分布。每个点对应一个构建（20 个 non-work、四个 work），横线表示中位数。S12 为每残基高置信度非局部接触特征 pp_nonlocal12_high_per_res。水平方向的偏移仅用于分开数据点，不代表定量差异。两类分布存在重叠，本图不用于确定通用分类阈值。*
...
*图 3. 五个构建在 C295、C388 和 C871 上的逐项预测—实验对照。每个构建分别在已保存的逐构建留一评价中留出，五者未同时留出。颜色表示各训练折选定阈值对应的类别：D，下降；ND，未跨越下降阈值；I，处于选定区间；U，上升。边框标出两处不一致：P9-GNS 的 C388 和 P8-GVE 的 C871。15 项相关联的终点预测中有 13 项一致。这五个构建是从已有实验结果中回顾性选出的重点案例。*

![fig3_model2_five_construct_confusion](figures/fig3_model2_five_construct_confusion.png)

*图 4. 同一组五个构建的终点混淆矩阵。C295、C388 和 C871 的一致数分别为 5/5、4/5 和 4/5。五个 C871 实验结果均属于下降类别，无法据此估计该终点的分类特异度。各构建分别留出，分类阈值由对应训练折确定。*
...
*图 6. 17 个共识非 TRM 替换在输入 PUF–RNA 结构中的位置。两个正交投影展示蛋白 Cα 轨迹（灰色）、RNA 磷原子轨迹（蓝色）和候选位置（橙色）。面板 b 的编号对应下方突变索引。坐标直接读取自 aice_mpnn_20260922/inputs/complex.pdb；残基编号遵循其 493 残基蛋白链 A。图中仅定位计算候选，尚不能据此确定其对结合、稳定性或编辑的影响。*
