...
root=Path(__file__).resolve().parents[2];manifest=json.loads((root/'research/docs/file_manifest.json').read_text());errors=[]
for row in manifest:
 p=root/row['path']
...
a=pd.read_csv(root/'research/architecture_validation/main_features.csv');assert len(a)==24 and a.success.sum()==4 and a.construct.nunique()==24
p=pd.read_csv(root/'research/architecture_validation/heldout_predictions.csv');p=p[p.subset=='PUF12_main'];assert p.groupby(['validation','feature_set','model']).construct.nunique().eq(24).all()
